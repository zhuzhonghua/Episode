...
try{
    ...
    if(self.inData && self.inData.length > 0){
...
Socket.prototype.sendByteBuffer = function(opcode, bufMsg){
    ...
    var holyMsg = new ByteBuffer(DEFINE.MSG_HEADER_SIZE+bufMsg.length);
    
    holyMsg.writeUInt16(opcode);
    var pack = (DEFINE.PACKET_HEAD << 24 >>> 0) + bufMsg.length;
    
    holyMsg.writeUInt32(pack);
    holyMsg.writeBuffer(bufMsg);
    this.outData.push(holyMsg.buffer);
    
    LOG(this._sockID + ': send msg ' + opcode + ' size ' + bufMsg.length);

    DEBUG_BYTE(holyMsg.buffer);
